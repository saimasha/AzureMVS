...
function get_confirmed_locations() {
    ...
    $query = "SELECT fdRetailerLat, fdRetailerLong, fdRetailerID FROM `tbRetailerMaster` WHERE fdDealerID = '".$_SESSION['fdRoleUniqueID']."' OR fdManufacturerID  = '".$_SESSION['fdRoleUniqueID']."' OR fdDistributorID = '".$_SESSION['fdRoleUniqueID']."' OR fdStockistID = '".$_SESSION['fdRoleUniqueID']."'";
    ...
    $query = "SELECT fdDealerLat, fdDealerLong, fdDealerID FROM `tbDealerMaster` WHERE fdDealerID = '".$_SESSION['fdRoleUniqueID']."' OR fdManufacturerID  = '".$_SESSION['fdRoleUniqueID']."' OR fdDistributorID = '".$_SESSION['fdRoleUniqueID']."' OR fdStockistID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
...
<div class="row">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title mb-0" style="font-family:Segoe UI,Arial,sans-serif;"><strong><i class="fas fa-map-marker-alt"></i>  DEALER LOCATION</strong></h4><br>
                    <div class="pac-card" id="pac-card">
                        <div id="label" style="color:white;font-family:Segoe UI,Arial,sans-serif;font-size: 0.7rem;">
                            Search Location
                        </div>       
...
autocomplete.addListener('place_changed', function() {
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        if (distance <= radius * 1000 && locations[i][3] === '50') {
...
for (i = 0; i < locations.length; i++) {
  var tbPowerFactor = locations[i][2] && locations[i][3];
...
// Add event listener for mouseover event
    ...
    //     infowindow.close();
    // });
    var markerIcon = locations[i][2] && locations[i][3] === '50' ? '\uf2b5' : '\uf0f8'; 
  var marker = new google.maps.Marker({
...
label: {
    ...
    "<td style='color:black;'><strong>" + (locations[i][3] === '50' ? "<strong> DEALER " + "</strong>" : "<strong> RETAILER " + "</strong>") + "</td></tr>\n" +
