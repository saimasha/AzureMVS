...
function get_confirmed_locations() {
    ...
    // distributor data
    $query = "SELECT fdDistrLat, fdDistrLong, fdDistributorID FROM `tbDistributorMaster` WHERE fdDistributorID = '".$_SESSION['fdRoleUniqueID']."' OR fdManufacturerID  = '".$_SESSION['fdRoleUniqueID']."' OR fdStockistID = '".$_SESSION['fdRoleUniqueID']."'";
    ...
    while ($r = mysqli_fetch_assoc($result)) {
        $locations[] = array($r['fdDistrLat'], $r['fdDistrLong'], $r['fdDistributorID'], '30'); // '30' represents distributor
    }

    // Retailer data
    $query = "SELECT fdRetailerLat, fdRetailerLong, fdRetailerID FROM `tbRetailerMaster` WHERE fdDistributorID = '".$_SESSION['fdRoleUniqueID']."' OR fdManufacturerID  = '".$_SESSION['fdRoleUniqueID']."'  OR fdStockistID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
    ...
    // dealer data
    $query = "SELECT fdDealerLat, fdDealerLong, fdDealerID FROM `tbDealerMaster` WHERE fdDistributorID = '".$_SESSION['fdRoleUniqueID']."' OR fdManufacturerID  = '".$_SESSION['fdRoleUniqueID']."'  OR fdStockistID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
...
<html>
<head>
  <title>Distributor Location</title>
...
<div class="row">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title mb-0" style="font-family:Segoe UI,Arial,sans-serif;"><strong><i class="fas fa-map-marker-alt"></i>  DISTRIBUTOR LOCATION</strong></h4><br>
                    <div class="pac-card" id="pac-card">
                        <div id="label" style="color:white;font-family:Segoe UI,Arial,sans-serif;font-size: 0.7rem;">
                            Search Location
                        </div>       
...
autocomplete.addListener('place_changed', function() {
    ...
    // Loop through the locations array to check if any distributors, dealers, or retailers exist within the radius
    for (var i = 0; i < locations.length; i++) {
        var distance = google.maps.geometry.spherical.computeDistanceBetween(
            new google.maps.LatLng(locationLat, locationLng),
            new google.maps.LatLng(locations[i][0], locations[i][1])
        );
        if (distance <= radius * 1000 && locations[i][3] === '30') {
    ...
    if (foundDistributor || foundDealer || foundRetailer) {
        // At least one relevant user (distributor, dealer, or retailer) found within the radius
...
label: {
    ...
    locations[i][3] === '50' ? "<strong> DEALER </strong>" : 
