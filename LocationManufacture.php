...
function get_confirmed_locations() {
    ...
    $query = "SELECT fdManufacturerLat, fdManufacturerLong, fdManufacturerID FROM `tbManufacturerMaster` WHERE fdManufacturerID= '".$_SESSION['fdRoleUniqueID']."'";
    ...
    while ($r = mysqli_fetch_assoc($result)) {
      $locations[] = array($r['fdManufacturerLat'], $r['fdManufacturerLong'], $r['fdManufacturerID'], '10'); 
  }

    // stockist data

    $query = "SELECT fdStockistLat, fdStockistLong, fdStockistID FROM `tbStockistMaster` WHERE fdManufacturerID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
    ...
    while ($r = mysqli_fetch_assoc($result)) {
      $locations[] = array($r['fdStockistLat'], $r['fdStockistLong'], $r['fdStockistID'], '20'); 
  }

    // distributor data

    $query = "SELECT fdDistrLat, fdDistrLong, fdDistributorID FROM `tbDistributorMaster` WHERE fdManufacturerID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
    ...
    while ($r = mysqli_fetch_assoc($result)) {
      $locations[] = array($r['fdDistrLat'], $r['fdDistrLong'], $r['fdDistributorID'], '30'); 
  }

    // Retailer data

    $query = "SELECT fdRetailerLat, fdRetailerLong, fdRetailerID FROM `tbRetailerMaster` WHERE fdManufacturerID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
    ...
    while ($r = mysqli_fetch_assoc($result)) {
      $locations[] = array($r['fdRetailerLat'], $r['fdRetailerLong'], $r['fdRetailerID'], '40'); 
  }

        // dealer data

    $query = "SELECT fdDealerLat, fdDealerLong, fdDealerID FROM `tbDealerMaster` WHERE fdManufacturerID = '".$_SESSION['fdRoleUniqueID']."'";
    $result = mysqli_query($conn, $query);
    ...
    while ($r = mysqli_fetch_assoc($result)) {
      $locations[] = array($r['fdDealerLat'], $r['fdDealerLong'], $r['fdDealerID'], '50'); 
  }
...
<div class="row">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title mb-0" style="font-family:Segoe UI,Arial,sans-serif;"><strong><i class="fas fa-map-marker-alt"></i>  MANUFACTURER LOCATION</strong></h4><br>
                    <div class="pac-card" id="pac-card">
                        <div id="label" style="color:white;font-family:Segoe UI,Arial,sans-serif;font-size: 0.7rem;">
                            Search Location
                        </div>       
...
autocomplete.addListener('place_changed', function() {
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        if (distance <= radius * 1000 && locations[i][3] === '10') {
            // Manufacturer found within the radius
            foundManufacturer = true;
        } else if (distance <= radius * 1000 && locations[i][3] === '20') {
    ...
    if (foundManufacturer || foundStockist || foundDistributor ||  foundDealer || foundRetailer) {
        ...
        // Manufacturer found within the radius, proceed with moving the manufacturer marker
        manufacturerMarker.setPosition(place.geometry.location);
        circle.setCenter(place.geometry.location);
        manufacturerMarker.setVisible(true);
        circle.setVisible(true);
        // infowindow.setContent('<div><strong>Manufacturer Found</strong><br>' + place.name + '<br>' + place.formatted_address + '</div>');
        // infowindow.open(map, manufacturerMarker);
    } else if (foundStockist) {
...
// var tooltipContent='';
  ...
  if (tbPowerFactor == "10") {
    color = '#e82020';
    markerLabel = '\uf1ad'; // Unicode for your first Font Awesome icon
...
label: {
    ...
    (locations[i][3] === '10' ? "<strong> MANUFACTURE </strong>" : 
