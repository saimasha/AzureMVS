                ...
                <script>
                  ...
                  function hide(id) {
                    ...
                    <tbody>
                        ...
                        $k=1;
                        if ($roleid === "MNFR"){
                            $sql ="SELECT * FROM tbDistributorMaster WHERE fdManufacturerID = '$RoleUniqueID'";
                        } elseif ($roleid === "STKS"){ 
                            $sql ="SELECT * FROM tbDistributorMaster WHERE fdStockistID = '$RoleUniqueID'";
                        } 
                        if ($result = mysqli_query($conn, $sql)) {
                    ...
                    <div  onMouseOver="show('hide<?php echo $k; ?>')" onMouseOut="hide('hide<?php echo $k; ?>')">
                     <a  href="?ViewDistributor&did=<?php echo $rows["fdDistributorID"];?>" style="color: #b2b9bf;"><?php echo $k; ?>
                    </a>
                     ...
                     <div id="NewNamehide<?php echo $k; ?>" class="scroll-sidebar">
                        ...
                        <tr><th>Latitude</th><td><?php echo $rows["fdDistrLat"]; ?></td></tr>
                        <tr><th>Longitude</th><td><?php echo $rows["fdDistrLong"]; ?></td></tr>
                        ...
                        <td style="color: <?php echo getStatusColor($rows['fdDistributorID'], $conn); ?>;"><?php echo getStatusButton($rows['fdDistributorID'], $conn); ?></td>
                         
                        <td> 
                            <form method="POST" class="d-flex justify-contain-center" >   
                                <button value="<?php echo $rows["fdDistributorID"]; ?>" name="btnDelete" id="btnDelete" class="d-inline btn btn-sm text-white m-1" type="submit" onclick="return confirmDelete();" style="background-color: #b60d0df7;"><i class="bi bi-trash-fill"
                                    ></i></button> 
                                <a href="?UpdateDistributor&did=<?php echo $rows["fdDistributorID"];?>">
                                <button  class="d-inline btn btn-sm text-white EditBtn m-1" name="BtnEdit" type="button" style="background-color: #044d0b;"><i class="bi bi-pencil-square"></i></button></a>       
...
<?php
function getStatusColor($DistributorId, $conn) {
    $sql = "SELECT fdStatus FROM tbUserMaster WHERE fdRoleUniqueID = '$DistributorId'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
    return $row['fdStatus'] == 1 ? 'green' : 'red';
}
   
function getStatusButton($DistributorId, $conn) {
    $sql = "SELECT fdStatus, fdEmailAsUserID FROM tbUserMaster WHERE fdRoleUniqueID = '$DistributorId'";
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_assoc($result);
   ...
   // Part of your existing code where you generate the status button
        if ($row['fdStatus'] == 1) {
            ...
            echo '<div class="text-center"><a href="updateStatus.php?DistributorId=' . $DistributorId . '&newStatus=0" class="btn btn-sm text-white custom-btn" style="background-color: #b60d0df7;" onclick="return confirm(\'Are you sure you want to make this user active? Send Email\');">Resend Email</a></div>';
        }
    } else {
        echo '<div class="text-center"><a href="updateStatus.php?DistributorId=' . $DistributorId . '&newStatus=0" class="btn btn-sm text-white custom-btn" style="background-color: #b60d0df7;" onclick="return confirm(\'Are you sure you want to make this user active? Send Email\');">Resend Email</a></div>';
    }
