...
<?php require "Customer_lognav.php"; ?>
...
$Email = $_POST['Email'];
$Sql = "SELECT COUNT(*) FROM tbCustomerMaster WHERE fdEmailAsUserID = '$Email'";
...
{
   sendEmailforgot1($conn, 30, $Email);
...
if (isset($_POST['resetBtn'])) {
    ...
    // Validate email (optional, consider using a dedicated validation library)
    ...
    if (empty($newpassword) || empty($confirmnewpassword)) {
        echo '<script>swal.fire({
            icon: "error",
            title: "Please enter both password and confirm password fields"
        })</script>';
        exit();
    }
    ...
    // Prepared statement to prevent SQL injection (highly recommended)
    $sql = "SELECT COUNT(*) FROM tbCustomerMaster WHERE fdEmailAsUserID = ?";
    ...
    if ($row[0] == 0) {
        ...
        // Hash password before storing (using a strong hashing algorithm like bcrypt)
        $hashedPassword = password_hash($newpassword, PASSWORD_BCRYPT);

        // Update query with prepared statement
        $sql = "UPDATE tbCustomerMaster SET fdPassword = ? WHERE fdEmailAsUserID = ?";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "ss", $hashedPassword, $email); // Bind hashed password and email parameters
