...
if(isset($_POST['login'])){
    $Email = $_POST['Email'];
    $Pass = $_POST['Pass'];
    $sql="SELECT * FROM tbCustomerMaster WHERE  fdEmailAsUserID = '$Email' AND fdPassword ='$Pass' ";

    $Result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($Result) > 0) {
        // Start a session and store customer ID
        $row = mysqli_fetch_assoc($Result);
        $_SESSION['fdCustomerID'] = $row['fdCustomerID'];

      echo '<script>
        window.location.href = "CustIndex.php";
        </script>';
    } else {
			...
			<div class="wrap-login100" style="opacity: 0.7;">
				...
				<form class="login100-form validate-form" method="POST">
					...
					<div class="text-center p-t-12">
						...
						<a class="txt2" href="Customer_forgetPass.php">
							Forgot Password
						</a> <br>
